(function(window,$,undefined) {
	...
	$.fn.sudokuJS=function(opts) {
		...
		opts=opts|| {};
		var solveMode=SOLVE_MODE_STEP,difficulty="unknown",candidatesShowing=false,editingCandidates=false,boardFinished=false,boardError=false,onlyUpdatedCandidates=false,gradingMode=false,generatingMode=false,invalidCandidates=[],strategies=[ {title:"openSingles",fn:openSingles,score:0.1}, {title:"singleCandidate",fn:singleCandidate,score:9}, {title:"visualElimination",fn:visualElimination,score:8}, {title:"nakedPair",fn:nakedPair,score:50}, {title:"pointingElimination",fn:pointingElimination,score:80}, {title:"hiddenPair",fn:hiddenPair,score:90}, {title:"nakedTriplet",fn:nakedTriplet,score:100}, {title:"hiddenTriplet",fn:hiddenTriplet,score:140}, {title:"nakedQuad",fn:nakedQuad,score:150}, {title:"hiddenQuad",fn:hiddenQuad,score:280}],usedStrategies=[],board=[],boardSize,boardNumbers,houses=[[],[],[]];
		...
		var initBoard=function(opts) {
			...
			if(!alreadyEnhanced) {
				for(var j=0; j<boardSize*boardSize; j++) {
					...
					board[j]= {val:cellVal,candidates:candidates};
				}
			}
		};
		...
		var updateUIBoardCell=function(cellIndex,opts) {
			...
			$("#input-"+cellIndex).val(newVal).addClass("highlight-val");
		...
		var clearBoard=function() {
			...
			for(var i=0; i<boardSize*boardSize; i++) {
				board[i]= {val:null,candidates:cands.slice()};
			}
			$boardInputs.removeClass("highlight-val").removeClass("board-cell--error").val("");
			updateUIBoard(false);
		};
